            ...
            <div class="w-full px-3 py-3 border rounded-md border-lightBlue">
              <template v-if="!Object.keys(selectedPlan).length">
                <select v-model="plan" required class="w-full select">
                  <option class="text-white bg-blue">Seleccion tú plan</option>
                  <option
              ...
              <template v-else>
                <input
                  v-model="selectedPlan.planName"
                  class="w-full px-3 py-3 text-white bg-blue"
                />
              </template>
            </div>
          </div>
      ...
      <div class="w-full">
        ...
        >
          <button
            ...
            @click="isCouponDisplay = !isCouponDisplay"
          >
        ...
        <Warning
          :is-open="warningOpen"
          :get-error="warningMessage"
          @close-warning="warningOpen = false"
        />
...
export default {
  ...
  data: () => ({
    ...
    successOpen: false,
    successMessage:
      'Felicitaciones tu solicitud fue enviada correctamente. Soporte se contactará contigo.',
    warningOpen: false,
    warningMessage:
      'Ooops, algo ocurrió. Contacta por teléfono, soporte estará arreglando este error pronto.',
  }),
  computed: {
    ...
    isCouponRight() {
      ...
      return (
        this.coupon !== this.showCoupon?.promotionCode &&
        this.coupon.length !== this.showCoupon?.promotionCode.length &&
        this.coupon.length
      )
    },
  },
  ...
  methods: {
    ...
    async sentData() {
      try {
        ...
        await axios.post(`${process.env.NUXT_API}api/hiringPlan`, request)
