  ...
  <div class="w-full flex flex-col justify-center items-center rounded-lg py-4 px-8 bg-blue">
    <div class="w-full flex flex-wrap justify-between items-center">
      ...
      <div class="w-full md:w-2/5 flex flex-col">
        ...
        <p v-for="(requirement, index) in vacancy.jobRequirements" :key="index" class="text-white font-light">
          * {{ requirement }}
        </p>
      </div>
    </div>
    <div class="w-full flex justify-between items-center mt-4 mb-2">
      <button v-if="!isInAdmin && !formIsOpen" @click="openCloseApplication"
        class="px-2 py-1 border border-grey rounded text-white">
        Enviar CV
      </button>
      <button v-if="isInAdmin" @click="deleteVacancy" class="px-2 py-1 bg-yellow rounded text-appBackground">
        Eliminar
      </button>
    </div>
