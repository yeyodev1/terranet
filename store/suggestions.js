...
const actions = {
  async fetchSuggestions({ commit }) {
    try {
      const response = await axios.get(`${process.env.NUXT_API}api/recommendations`,
      {
        headers: {
  ...
  async deleteSuggestions({ commit, dispatch }, payload) {
    try {
      const response = await axios.delete(`${process.env.NUXT_API}api/recommendations/${payload}`,
        {
          headers: {
              Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))}`
          }
