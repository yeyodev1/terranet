...
const actions = {
  async fetchChangeLocation({ commit }) {
    try {
      const response = await axios.get(`${process.env.NUXT_API}api/homeTransferReq`,
      {
        headers: {
  ...
  async deleteChangeLocation({ commit, dispatch }, payload) {
    try {
      const response = await axios.delete(`${process.env.NUXT_API}api/homeTransferReq/${payload}`,
        {
          headers: {
              Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))}`
          }
