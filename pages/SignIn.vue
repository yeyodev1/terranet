<template>
  <div class="mx-auto w-full flex items-start min-h-screen bg-appBackground">
    <div class="w-full mx-auto flex flex-col items-center max-w-lg mt-10">
      <h1 class="text-white text-xl font-light mb-10">
        Iniciar Sesión
      </h1>
      <span v-if="getError.length" class="w-full flex justify-center items-center py-2 bg-red text-white my-4">
        {{ getError }}
      </span>
      <div class="w-full mx-auto flex flex-col items-center">
        <div class="inputBox mb-7">
          <input v-model="credentials.email" type="text" autocomplete="off" required>
          <span>Correo</span>
        ...
        <div class="inputBox mt-20">
          <input v-model="credentials.password" type="password" autocomplete="off" required>
...
export default {
  ...
  computed: {
    ...
    passwordIsValid() {
      return !this.credentials.password.length ? this.validatingPassword() : false;
    },
    formIsValid() {
      return !this.emailIsValid.length && !this.passwordIsValid && this.credentials.email.length && this.credentials.password
    }
  },
  methods: {
    ...
    validatingEmail() {
      return !emailValidation(this.credentials.email) ? 'Ingresa un correo válido' : ''
    },
    validatingPassword() {
