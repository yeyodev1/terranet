<template>
  <app-layout>
    <app-title title="Capacidad" class="mt-4" />
      ...
      <div v-if="isOpen" class="w-full flex flex-col justify-start mt-6">
        <div class="w-full flex flex-col justify-between items-center pl-1">
          <div class="w-full flex flex-col justify-center items-start">
            <label class="text-white font-primary text-lg mb-3">Subir Archivos
            </label>
            <button class="relative p-2 border border-lightBlue rounded-lg">
              ...
              <input type="file" class="opacity-0 input-scale" @change="onImageChange" />
            </button>
            <div v-if="image.url.length" class="w-16 h-16 flex justify-center items-center mt-2">
              <img :src="image.url" alt="" class="w-16 h-16 object-cover" />
            </div>
          </div>
        </div>
        <button @click="setImage" class="w-28 rounded-lg flex justify-center items-center text-appBackground py-1 mt-4"
          :class="{
    ...
    <div v-if="getCapacity.length" class="mt-4 flex p-4 justify-center items-center border border-lightBlue rounded-lg">
      <img :src="getCapacity" class="mx-auto w-4/5" />
    </div>
  </app-layout>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters('capacity', ['getCapacity']),
    arrowDisplay() {
  ...
  methods: {
    ...mapActions('capacity', ['fetchCapacity', 'setCapacity']),
    showCard() {
