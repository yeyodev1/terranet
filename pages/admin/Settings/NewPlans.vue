<template>
  <div>
        ...
        <div v-if="isOpen" class="w-full flex flex-col justify-start mt-6">
          <div class="w-full flex flex-wrap justify-between items-center pl-1">
            <div class="w-full lg:w-2/5 flex justify-start items-center">
              <label class="text-white font-primary text-lg mr-1">Nombre:
              </label>
              <input v-model="namePlan" placeholder="Ingresa el nombre" type="text"
            ...
            <div class="w-full sm:w-1/2 lg:w-1/5 flex justify-start items-center mt-2 lg:mt-0">
              <label class="text-white font-primary text-lg mr-1">Precio:
              </label>
              <input v-model="pricePlan" placeholder="$ 00.00" type="text"
                class="text-white outline-none border-b border-lightBlue w-20 px-1 bg-appBackground" />
            </div>
            <div class="w-full sm:w-1/2 lg:w-1/5 flex justify-start items-center mt-2 lg:mt-0">
              <label class="text-white font-primary text-lg mr-1">Velocidad:
              </label>
              <input v-model="speedPlan" placeholder="00" type="text"
                class="text-white outline-none border-b border-lightBlue px-1 bg-appBackground w-9" />
            </div>
          </div>
          <button @click="savePlan" class="w-28 rounded-lg flex justify-center items-center text-appBackground py-1 mt-4"
            :class="{
    ...
    <p v-if="!getPlans.length" class="font-primary text-white text-3xl w-full text-center mt-4 font-extralight">
      Aún no se han creado planes
    </p>
    <div v-else class="w-full mx-auto flex flex-wrap justify-evenly">
      <div v-for="(plan, i) in getPlans" :key="i" class="w-full sm:w-2/5 md:mr-4 mt-6 px-2">
