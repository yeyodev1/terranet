  ...
  <app-layout>
    <template>
      ...
      <div class="flex flex-col items-center w-full">
        <div class="w-full border border-lightBlue rounded-lg mt-6 p-6">
          ...
          <div v-if="isOpen" class="w-full flex flex-col justify-start mt-6">
            <div class="w-full flex flex-wrap justify-between items-center pl-1">
              <div class="w-full sm:w-1/2 lg:w-1/4 flex justify-start items-center mt-2 lg:mt-0">
                <label class="text-white font-primary text-lg mr-1">Nombre del Codigo:
                </label>
                <input v-model="name" placeholder="Terranet 2020" type="text"
                  class="text-white outline-none border-b border-lightBlue w-36 px-1 bg-appBackground" />
              </div>
              <div class="w-full sm:w-1/2 lg:w-1/5 flex justify-start items-center mt-2 lg:mt-0">
                <label class="text-white font-primary text-lg mr-1">Porcentaje:
                </label>
                <input v-model="discount" placeholder="00" type="text"
                  class="text-white outline-none border-b border-lightBlue px-1 bg-appBackground w-9" />
              </div>
            </div>
            <button @click="setCoupon"
              class="w-28 rounded-lg flex justify-center items-center text-appBackground py-1 mt-4" :class="{
        ...
        <div v-if="Object.keys(showCoupon).length" class="w-full mt-8 flex justiify-center items-center">
          <Coupon class="mx-auto" :name="showCoupon.promotionCode" :discount="showCoupon.discountPercentage"
            :id="showCoupon._id" @delete-coupon="removeCoupon(showCoupon._id)" />
        </div>
      </div>
    </template>
  </app-layout>
