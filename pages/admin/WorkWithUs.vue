<template>
  <app-layout>
    ...
    <div class="border border-lightBlue rounded-lg mt-6 p-6">
      <button class="w-full flex justify-between items-center" @click="showCard">
      ...
      <div v-if="isOpen" class="w-full flex flex-col justify-start mt-6">
        <div class="w-full flex flex-wrap justify-between items-center pl-1">
          <div class="w-full flex justify-start items-start">
            <label class="text-white font-primary text-lg mr-1">Título:
            </label>
            <input v-model="jobTitle" placeholder="Ingresa el nombre" type="text"
          ...
          <div class="w-full lg:w-1/2 flex flex-col justify-start items-start mt-4">
            <label class="text-white font-primary text-lg mr-1">Descripción:
            </label>
            <textarea v-model="jobDescription" placeholder="Descripcion" type="text"
              class="w-full lg:w-3/4 text-white outline-none border-b border-lightBlue px-1 bg-appBackground" />
          </div>
          <div class="w-full lg:w-1/2 flex flex-col justify-start items-start mt-4">
            <label class="text-white font-primary text-lg mr-1">Requesitos:
            </label>
            <input v-model="jobRequirements" @keyup.enter="getRequirement" placeholder="Requisito" type="text"
          ...
          <div class="w-full flex justify-start items-baseline mt-3">
            <p class="text-white mr-2">Requisitos:</p>
            <div v-if="requirements.length" class="w-full flex flex-wrap mt-3">
              <div v-for="(requirement, i) in requirements" :key="i"
                ...
                <span class="text-appBackground mr-1">{{ requirement }}
                </span>
                <button @click="removeRequirement(i)" class="rounded-full bg-red w-3 h-3">
        ...
        <button @click="saveVacancy"
          class="w-28 rounded-lg py-1 px-2 flex justify-center items-center text-appBackground mt-6" :class="{
    ...
    <div v-if="getVacancies.length" class="mt-4">
      <vacancy-card v-for="(vacancy, index) in getVacancies" :key="index" :vacancy="vacancy" @delete-vacancy="removeJob"
        class="mb-4" />
    </div>
  </app-layout>
</template>
