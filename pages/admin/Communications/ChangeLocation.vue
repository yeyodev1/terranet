<template>
  <app-layout>
    ...
    <div v-if="getChangeLocation.length" class="w-full flex flex-col justify-center items-center mt-4">
      <div v-for="(owner, index) in getChangeLocation" :key="index"
        ...
        <div class="w-full flex justify-evenly items-start flex-wrap">
          <div class="w-full flex flex-col">
            <app-title :title="`Cédula del cliente ${owner.idUser}`" class="mt-4" />
            <div class="w-full flex flex-col items-start mt-2">
              ...
              <div v-if="owner.gpsLocation !== undefined" class="w-full flex justify-start items-center mb-2">
                <p class="text-white font-bold mr-2">
                  Ubicación GPS:
                </p>
                <p class="text-white font-light mr-2">
        ...
        <button class="border border-yellow rounded-md px-2 py-1 text-white mt-3" @click="openModal(owner._id)">
          Eliminar
        </button>
        <deleting-modal :isOpen="isOpen" @close-modal="closeModal(owner._id)"
