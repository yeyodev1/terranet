<template>
  <app-layout>
    ...
    <div v-if="getSuggestions.length" class="w-full flex flex-col justify-center items-center mt-4">
      <div v-for="(suggestion, index) in getSuggestions" :key="index"
        ...
        <div class="w-full flex flex-wrap items-start justify-between">
          <div class="w-full lg:w-3/5 flex flex-wrap justify-center items-start my-3">
            <deleting-modal :isOpen="isOpen" @close-modal="closeModal(suggestion._id)"
        ...
        <button class="border border-yellow rounded-md px-2 py-1 text-white mt-3" @click="openModal(suggestion._id)">
          Eliminar
        </button>
      </div>
    </div>
